...
public class GUISolarWirelessMachinesCharger extends GuiIC2<ContainerSolarWirelessMachinesCharger> {
    ...
    public GUISolarWirelessMachinesCharger(ContainerSolarWirelessMachinesCharger container) {
        super(container, 196);
        this.thisBase = (TileEntitySolarMachinesCharger)container.base;
        this.addElement(EnergyGauge.asBar(this, 75, 40, this.thisBase));
        ...
        /*this.addElement(new CustomButton(this, 92, 68, 11, 11, this.createEventSender(1)).withTooltip("+1"));
        this.addElement(new CustomButton(this, 72, 68, 11, 11, this.createEventSender(2)).withTooltip("-1"));
        this.addElement(new CustomWIButtonBT(this, 57, 17, 60, 15, mouseButton -> {
            switch(mouseButton) {
                case left:
                    IC2.network.get(false).initiateClientTileEntityEvent(this.thisBase, 3);
                    break;
                case right:
                    IC2.network.get(false).initiateClientTileEntityEvent(this.thisBase, 4);
                    break;
            }
        }).withText(() -> this.thisBase.getMode() == 8
                ? Localization.translate("gui.wchm.mode.automatic")
                : (int)this.thisBase.getChargeRate() + " EU/t").withTooltip("gui.wchm.to.change.mode"));
        
        this.addElement(new CustomButton(this, 37, 36, 20, 20, this.createEventSender(5))
                .withTooltip(Localization.translate("gui.dispatcher.tooltip.switch")));*/
        this.addElement(new Area(this, 121, 18, 14, 14).withTooltip(() -> {
            switch(this.thisBase.getGenState()) {
                case DAY:
                    return Localization.translate("gui.WP.generating") + ": " + String.format("%.0f", this.thisBase.getDayPower()) + " EU/t";
                case NIGHT:
                    return Localization.translate("gui.WP.generating") + ": " + String.format("%.0f", this.thisBase.getNightPower()) + " EU/t";
                default:
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        this.drawTexturedModalRect(this.guiLeft + 56, this.guiTop + 39,
                this.thisBase.getIsOn() ? 221 : 235, 8, 11, 11);
        ...
        switch(this.thisBase.getGenState()) {
            case DAY:
                this.drawTexturedModalRect(this.guiLeft + 121, this.guiTop + 18, 177, 25, 14, 14);
                break;
            case NIGHT:
                this.drawTexturedModalRect(this.guiLeft + 121, this.guiTop + 18, 191, 25, 14, 14);
                break;
            case NONE:
                this.drawTexturedModalRect(this.guiLeft + 121, this.guiTop + 17, 205, 25, 14, 14);
                break;
        }
    ...
    protected void drawForegroundLayer(int mouseX, int mouseY) {
        ...
        String radiusstring = Localization.translate("gui.WChM.radius") + ": "
                + this.thisBase.getRadius() + " " + Localization.translate("gui.blocks");
        String offsetHorizontal = Localization.translate("gui.wchm.offset.xz") + ": "
                + this.thisBase.getOffsetXZ() + " " + Localization.translate("gui.blocks");
        String offsetVertical = Localization.translate("gui.wchm.offset.y") + ": "
                + this.thisBase.getOffsetY() + " " + Localization.translate("gui.blocks");
        if(this.thisBase.getIsAsymmetricCharging()) {
        ...
        String tilesQuantity = Localization.translate("gui.dispatcher.tiles.quantity.radius") + ": "
                + this.thisBase.getEnergyTilesQuantity();
